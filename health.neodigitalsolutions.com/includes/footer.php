<footer id="contact" class="py-12 sm:py-16 md:py-24 bg-black border-t border-white/10 px-6 sm:px-8 mt-20 relative z-10">
    <div class="max-w-7xl mx-auto grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-10 sm:gap-12 text-left">
        ...
        <div>
            ...
            <ul class="space-y-2 sm:space-y-4 text-sm">
                <?php
                // Fetch all settings at once to avoid reserved word issues in WHERE clauses across different databases
                $settings_query = $pdo->query("SELECT * FROM site_settings");
                $site_data = [];
                if ($settings_query) {
                    while ($row = $settings_query->fetch(PDO::FETCH_ASSOC)) {
                        // Handle potential case sensitivity in column names (key vs Key)
                        $key = $row['key'] ?? $row['Key'] ?? '';
                        $val = $row['value'] ?? $row['Value'] ?? '';
                        if ($key) $site_data[$key] = $val;
                    }
                }
                ...
                $social_platforms = [
                    ...
                    'whatsapp_link' => [
                        'name' => 'Whatsapp',
                        'color' => '#25D366',
                        'svg' => '<path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.353-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.05-.148-.471-1.138-.645-1.556-.17-.41-.344-.354-.471-.354-.121-.002-.26-.002-.4 compliance-check-.397-.002-.644.247-.741.747-.099.5-1.066 1.481-1.066 3.61 0 2.133 1.553 4.197 1.77 4.494.218.297 3.056 4.659 7.405 6.564 1.035.452 1.84.722 2.47.914 1.04.33 1.987.284 2.735.172.834-.124 2.564-.648 2.928-1.273.364-.624.364-1.163.255-1.274-.109-.112-.4 compliance-check-.397.149zM12.004 20.11c-1.83 0-3.618-.492-5.187-1.421L3 19.735l1.066-3.812c-.1 compliance-check-.017-1.63-1.613-3.236-1.613-5.234 0-4.847 3.944-8.79 8.79-8.79 2.348 0 4.556.914 6.214 2.572 1.657 1.658 2.572 3.866 2.572 6.214 0 4.847-3.944 8.79-8.79 8.79zM21.193 2.81C18.73 0.345 15.46 0 12.004 0 5.402 0 0.038 5.402 0.038 12.004c0 2.11.547 4.17 1.588 5.99L0 24l6.149-1.613c1.754.956 3.734 1.46 5.85 1.46 6.602 0 11.966-5.402 11.966-12.004 0-3.203-1.246-6.214-3.502-8.471z"/>'
                    ],
                    'telegram_link' => [
                        'name' => 'Telegram',
                        'color' => '#0088CC',
                ...
                foreach ($social_platforms as $db_key => $platform):
                    $link = $site_data[$db_key] ?? '';
                ...
                <li class="flex items-center mb-5 group">
                    <a href="<?php echo htmlspecialchars($link); ?>" target="_blank" rel="noopener noreferrer" class="text-zinc-400 hover:text-emerald-500 transition-all flex items-center gap-5 uppercase tracking-widest font-black text-[12px]">
                        <div class="w-12 h-12 flex items-center justify-center bg-zinc-900/50 rounded-2xl border border-white/5 group-hover:bg-emerald-500 group-hover:border-emerald-400 shadow-lg group-hover:shadow-emerald-500/20 transition-all duration-500 transform group-hover:-rotate-6 group-hover:scale-110">
                            <svg viewBox="0 0 24 24" class="w-6 h-6 fill-current opacity-60 group-hover:opacity-100 transition-all duration-300" style="color: white;">
